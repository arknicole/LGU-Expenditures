...
function populateSubSubPpa(ppaSelectId, subPpaSelectId, subSubPpaSelectId, sourceCache, shouldShowExisting = false) {
    ...
    if (!selectedPpa || !selectedSubPpa) populateDropdown(subSubPpaSelect, [], '---');
    else {
...
function addPPA() {
    ...
    const amount = parseFloat(document.getElementById('ppa_amount').value);
    if (!selectedCategory || !selectedCategory.id) return alert('Please select a complete PPA.');
    if (isNaN(amount) || amount <= 0) return alert('Invalid amount.');
    addToList(ppas, { category_id: selectedCategory.id, name: formatCategoryName(selectedCategory), amount: amount });
    renderList('ppas_list', ppas);
    document.getElementById('ppa_amount').value = '';
}
...
function addFromPPA() {
    ...
    const pendingAmount = fromPPAs.filter(p => p.category_id === selectedCategory.id).reduce((sum, p) => sum + p.amount, 0);
    if (amount > (currentDbBalance - pendingAmount)) {
        return alert(`Insufficient Balance! Available: ${formatCurrency(currentDbBalance - pendingAmount)}`);
    }
...
function addToPPA() {
    ...
    const amount = parseFloat(document.getElementById('to_amount').value);
...
function handleListClick(event, listId) {
    ...
    if (event.target.textContent === '[Remove]') {
        const list = (listId === 'ppas_list') ? ppas : (listId === 'from_list' ? fromPPAs : toPPAs);
        list.splice(index, 1);
...
async function submitAllotment() { 
    ...
    try {
        const res = await fetch('/api/allotment/add', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
        const data = await res.json();
        alert(data.message);
        if (data.success) {
...
function formatCurrency(val) { return parseFloat(val || 0).toLocaleString('en-PH', {minimumFractionDigits: 2}); }
    ...
    try {
        ...
        if (data.success && data.allotments) {
            data.allotments.forEach(row => {
                ...
                let displayAmount = row.is_from === 1 ? -row.amount : row.amount;
                tbody.innerHTML += `<tr><td>${row.year}</td><td>${row.type}</td><td>${row.office_name}</td><td><small>${row.ppa} ${row.sub_ppa ? '> '+row.sub_ppa : ''}</small></td><td style="text-align:right;">${parseFloat(displayAmount).toLocaleString('en-PH', {minimumFractionDigits:2})}</td><td style="text-align:center;"><button onclick='openEditModal(${safeItem})' style="background:#ffc107; border:none; cursor:pointer; padding:3px 6px; border-radius:3px;"><i class="fas fa-edit"></i></button><button onclick="deleteItem(${row.detail_id})" style="background:#dc3545; color:white; border:none; cursor:pointer; padding:3px 6px; border-radius:3px; margin-left:5px;"><i class="fas fa-trash"></i></button></td></tr>`;
            });
        }
...
async function saveEdit() {
    ...
    try {
        const res = await fetch('/api/allotment/update', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ id, amount }) });
        const data = await res.json();
        if(data.success) { alert('Updated!'); document.getElementById('editModal').style.display = 'none'; loadRecentTable(); } else alert(data.message);
...
async function deleteItem(id) {
    ...
    try {
        const res = await fetch('/api/allotment/delete', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ id }) });
        const data = await res.json();
        if(data.success) loadRecentTable(); else alert(data.message);
